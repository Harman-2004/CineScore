...
def calculate_normalized_weights(
    ...
    youtube_val: Optional[Union[float, str]] = None
) -> Tuple[float, Dict[str, float], List[str], List[str]]:
    """
    Intelligently normalizes rating weights proportionally when some sources are missing.
    
    Default weights (YouTube prioritized as highest):
    YouTube Comments = 40% (0.40)
    IMDb = 25% (0.25)
    NLP Sentiment = 15% (0.15)
    TMDb = 10% (0.10)
    Metacritic = 10% (0.10)
    ...
    parsed_values = {
        ...
        "youtube": safe_float(youtube_val)
    }
